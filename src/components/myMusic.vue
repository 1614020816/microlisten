<template>
    <div id="g-mymusic" class="g-mymusic">
        <div class="g-bd3 p-mymusic f-cb">
            <div class="g-sd3 u-scroll n-musicsd f-pr j-flag">
                <div id="my-music">
                    <h2 id="m-sub-artist" class="usersub">
                    	<a href="#" class="f-fs1 s-fc0 f-ff1">我的歌手</a>
                    </h2>
                    <div class="n-minelst n-minelst-1" style="margin-top: 8px;">
                    	<h2 class="f-ff1">
                    		<a href="javascript:;" class="u-btn u-btn-crt f-fr j-flag" data-action="create-playlist">
                    			<i>新建</i>
                    		</a>
                    		<span class="rtitle" data-action="spread">
                    			<i class="tri tri1"></i>创建的歌单(<span class="j-flag">2</span>)
                    		</span>
                    	</h2>
                    	<ul class="j-flag f-cb">
                    		<li class="j-iflag z-selected" id="like-music" data-matcher="playlist-431357805">
                    			<div class="item f-cb">
                    				<div class="left">
                    					<a href="#" hidefocus="true" class="avatar">
                    						<img src="../assets/img/myMusic/likesmusic.jpg"/>
                    					</a>
                    				</div>
                    				<p class="name f-thide">
                    					<a href="javascript:;" hidefocus="true" class="s-fc0" title="我喜欢的音乐">我喜欢的音乐</a>
                    				</p>
                    				<p class="s-fc4 f-thide num">229首</p>
                    			</div>
                    			<span class="oper hshow" ></span>
                    		</li>
                    		<li class="j-iflag" id="like-music" data-matcher="playlist-431357805">
                    			<div class="item f-cb">
                    				<div class="left">
                    					<a href="#" hidefocus="true" class="avatar">
                    						<img src="../assets/img/myMusic/ting.jpg"/>
                    					</a>
                    				</div>
                    				<p class="name f-thide">
                    					<a href="javascript:;" hidefocus="true" class="s-fc0" title="婷">婷</a>
                    				</p>
                    				<p class="s-fc4 f-thide num">11首</p>
                    			</div>
                    			<span class="oper hshow">
                    				<a href="#" hidefocus="true" title="编辑" class="u-icn u-icn-10"></a>
                    				<a href="#" data-action="delect" hidefocus="true" title="删除" class="u-icn u-icn-11"></a>
                    			</span>
                    		</li>
                    	</ul>
                    </div>
                	<div class="n-minelst n-minelst-1">
                		<h2 class="f-ff1">
                    		<span class="rtitle" data-action="spread">
                    			<i class="tri tri1"></i>
                    			收藏的歌单(
                    			<span class="j-flag">15</span>
                    			)
                    		</span>
                    	</h2>
                    	<ul class="j-flag f-cb">
                    		<li class="j-iflag" id="like-music" data-matcher="playlist-431357805">
                    			<div class="item f-cb">
                    				<div class="left">
                    					<a href="#" hidefocus="true" class="avatar">
                    						<img src="../assets/img/myMusic/likesmusic.jpg"/>
                    					</a>
                    				</div>
                    				<p class="name f-thide">
                    					<a href="javascript:;" hidefocus="true" class="s-fc0" title="那些好听到发疯的粤语歌">那些好听到发疯的粤语歌</a>
                    				</p>
                    				<p class="s-fc4 f-thide num">360首 &nbsp;by 天寒地凍路遠馬亡又何妨</p>
                    			</div>
                    			<span class="oper hshow">
                    				<a href="#" data-action="delect" hidefocus="true" title="删除" class="u-icn u-icn-11"></a>
                    			</span>
                    		</li>
                    		<li class="j-iflag" id="like-music" data-matcher="playlist-431357805">
                    			<div class="item f-cb">
                    				<div class="left">
                    					<a href="#" hidefocus="true" class="avatar">
                    						<img src="../assets/img/myMusic/likesmusic.jpg"/>
                    					</a>
                    				</div>
                    				<p class="name f-thide">
                    					<a href="javascript:;" hidefocus="true" class="s-fc0" title="那些好听到发疯的粤语歌">那些好听到发疯的粤语歌</a>
                    				</p>
                    				<p class="s-fc4 f-thide num">360首 &nbsp;by 天寒地凍路遠馬亡又何妨</p>
                    			</div>
                    			<span class="oper hshow">
                    				<a href="#" data-action="delect" hidefocus="true" title="删除" class="u-icn u-icn-11"></a>
                    			</span>
                    		</li>
                    		<li class="j-iflag" id="like-music" data-matcher="playlist-431357805">
                    			<div class="item f-cb">
                    				<div class="left">
                    					<a href="#" hidefocus="true" class="avatar">
                    						<img src="../assets/img/myMusic/likesmusic.jpg"/>
                    					</a>
                    				</div>
                    				<p class="name f-thide">
                    					<a href="javascript:;" hidefocus="true" class="s-fc0" title="那些好听到发疯的粤语歌">那些好听到发疯的粤语歌</a>
                    				</p>
                    				<p class="s-fc4 f-thide num">360首 &nbsp;by 天寒地凍路遠馬亡又何妨</p>
                    			</div>
                    			<span class="oper hshow">
                    				<a href="#" data-action="delect" hidefocus="true" title="删除" class="u-icn u-icn-11"></a>
                    			</span>
                    		</li>
                    		<li class="j-iflag" id="like-music" data-matcher="playlist-431357805">
                    			<div class="item f-cb">
                    				<div class="left">
                    					<a href="#" hidefocus="true" class="avatar">
                    						<img src="../assets/img/myMusic/likesmusic.jpg"/>
                    					</a>
                    				</div>
                    				<p class="name f-thide">
                    					<a href="javascript:;" hidefocus="true" class="s-fc0" title="那些好听到发疯的粤语歌">那些好听到发疯的粤语歌</a>
                    				</p>
                    				<p class="s-fc4 f-thide num">360首 &nbsp;by 天寒地凍路遠馬亡又何妨</p>
                    			</div>
                    			<span class="oper hshow">
                    				<a href="#" data-action="delect" hidefocus="true" title="删除" class="u-icn u-icn-11"></a>
                    			</span>
                    		</li>
                    		<li class="j-iflag" id="like-music" data-matcher="playlist-431357805">
                    			<div class="item f-cb">
                    				<div class="left">
                    					<a href="#" hidefocus="true" class="avatar">
                    						<img src="../assets/img/myMusic/likesmusic.jpg"/>
                    					</a>
                    				</div>
                    				<p class="name f-thide">
                    					<a href="javascript:;" hidefocus="true" class="s-fc0" title="那些好听到发疯的粤语歌">那些好听到发疯的粤语歌</a>
                    				</p>
                    				<p class="s-fc4 f-thide num">360首 &nbsp;by 天寒地凍路遠馬亡又何妨</p>
                    			</div>
                    			<span class="oper hshow">
                    				<a href="#" data-action="delect" hidefocus="true" title="删除" class="u-icn u-icn-11"></a>
                    			</span>
                    		</li>
                    	</ul>
                	</div>
                	<div style="height: 100px;"></div>
                </div>
            </div>
            <div class="g-mn3 f-pr j-flag">
            	<div class="f-pr">
            		<div class="j-flag" id="likeMusicTop">
            			<div class="g-wrap">
            				<div class="m-info f-cb">
            					<div class="cover u-cover u-cover-dj">
            						<img src="../assets/img/myMusic/likemusictop.jpg" id="flag_cover"/>
            						<span class="msk"></span>
            					</div>
            					<div class="cnt">
									<div class="cntc m-info">
										<div class="hd f-cb">
											<i class="type u-icn u-icn-13"></i>
											<h2 class="f-ff2 f-thide">我喜欢的音乐</h2>
										</div>
										<div class="user f-cb">
											<a href="#" class="face">
												<img src="../assets/img/myMusic/personphoto.jpg"/>
											</a>
											<span class="name f-thide">
												<a href="#" class="s-fc7" title="丶Yejishzhying">丶Yejishzhying</a>
											</span>
											<span class="time s-fc4">2016-07-27&nbsp;创建</span>
										</div>
										<div class="btns f-cb" id="flag_play_addto_btn_wrapper">
											<a href="javascript:;" class="u-btn2 u-btn2-2 u-btni-addply f-fl" hidefocus="true" title="播放" data-res-type="13" data-res-id="id" data-res-action="play" data-res-from="31" data-res-data="2222">
												<i>
													<em class="ply"></em>
													播放
												</i>
											</a>
											<a href="javascript:;" class="u-btni u-btni-add" hidefocus="true" title="添加到播放列表" data-res-type="13" data-res-id="id" data-res-action="addto" data-res-from="31" data-res-data="2222"></a>
											<a id="toplist-fav" href="javascript:;" class="u-btni u-btni-fav" hidefocus="true" data-res-type="13" data-res-id="id" data-res-action="fav" data-res-from="31" data-res-data="2222">
												<i>收藏</i>
											</a>
											
											<a id="toplist-share" href="javascript:;" class="u-btni u-btni-share" hidefocus="true" data-res-type="13" data-res-id="id" data-res-action="share" data-res-name="网易原创歌曲榜" data-res-author="原创君" data-res-data="2222">
												<i>分享</i>
											</a>
											<a href="javascript:;" class="u-btni u-btni-dl" data-res-type="13" data-res-id="id" data-res-action="download" >
												<i>下载</i>
											</a>
											<a href="javascript:;" class="u-btni u-btni-cmmt j-cmt" data-res-id="id" data-res-action="comment" >
												<i>评论</i>
											</a>
										</div>
									</div>
								</div>
            				</div>
            			</div>
            			<div class="u-title u-title-1 f-cb">
							<h3>
								<span class="f-ff2">歌曲列表</span>
							</h3>
							<span class="sub s-fc3">
								<span>98</span>
								首歌
							</span>
							<div class="more s-fc3">
								播放：
								<strong class="s-fc6" id="play-count">44</strong>
								次
							</div>
            			</div>
            		</div>
            		<div class="j-flag">
            			<div id="songList">
							<div class="j-flag" id="songListTop">
								<table class="m-table">
								  	<thead>
								  		<tr>
								  			<th class="first w1">
								  				<div class="wp">
								  					&nbsp;
								  				</div>
								  			</th>
								  			<th>
								  				<div class="wp">
								  					歌曲标题
								  				</div>
								  			</th>
								  			<th class="w2">
								  				<div class="wp">
								  					时长
								  				</div>
								  			</th>
								  			<th class="w3">
								  				<div class="wp">
								  					歌手
								  				</div>
								  			</th>
								  			<th class="w4">
								  				<div class="wp">
								  					专辑
								  				</div>
								  			</th>
								  		</tr>
								  	</thead>
								  	<tbody>
								  		<tr id="songid" class="even">
								  			<td class="left">
								  				<div class="hd">
								  					<span data-res-id="22222" data-res-type="18" data-res-action="play" class="ply">&nbsp;</span>
								  					<span class="num">1</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="f-cb">
								  					<div class="tt">
								  						<div class="ttc">
								  							<span class="txt">
								  								<a href="#">
								  									<b title="为什么我泪水成行">为什么我泪水成行</b>
								  								</a>
								  							</span>
								  						</div>
								  					</div>
								  				</div>
								  			</td>
								  			<td class="s-fc3">
								  				<span class="u-dur candel">04:40</span>
								  				<div class="opt hshow">
								  					<a href="javascript:;" class="u-icn u-icn-81 icn-add" title="添加到播放列表" hidefocus="true" data-res-type="18" data-res-action="addto"></a>
								  					<span class="icn icn-fav" title="收藏" data-res-id="516320327" data-res-type="18" data-res-action="fav"></span>
								  					<span class="icn icn-share" data-res-action="share" title="分享" data-res-type="18" data-res-name="为什么我泪水成行" data-res-author="刀郎">分享</span>
								  					<span class="icn icn-dl" title="下载" data-res-action="download" data-res-type="18">下载</span>
								  					<span class="icn icn-del" title="删除" data-res-action="delete" data-res-type="18">下载</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text" title="刀郎">
								  					<span title="刀郎">
								  						<a href="#" hidefocus="true">刀郎</a>
								  					</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text">
								  					<a href="#" title="谢谢你">谢谢你</a>
								  				</div>
								  			</td>
								  		</tr>
								  		<tr id="songid">
								  			<td class="left">
								  				<div class="hd">
								  					<span data-res-id="22222" data-res-type="18" data-res-action="play" class="ply">&nbsp;</span>
								  					<span class="num">2</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="f-cb">
								  					<div class="tt">
								  						<div class="ttc">
								  							<span class="txt">
								  								<a href="#">
								  									<b title="为什么我泪水成行">为什么我泪水成行</b>
								  								</a>
								  							</span>
								  						</div>
								  					</div>
								  				</div>
								  			</td>
								  			<td >
								  				<span class="u-dur candel">04:40</span>
								  				<div class="opt hshow">
								  					<a href="javascript:;" class="u-icn u-icn-81 icn-add" title="添加到播放列表" hidefocus="true" data-res-type="18" data-res-action="addto"></a>
								  					<span class="icn icn-fav" title="收藏" data-res-id="516320327" data-res-type="18" data-res-action="fav"></span>
								  					<span class="icn icn-share" data-res-action="share" title="分享" data-res-type="18" data-res-name="为什么我泪水成行" data-res-author="刀郎">分享</span>
								  					<span class="icn icn-dl" title="下载" data-res-action="download" data-res-type="18">下载</span>
								  					<span class="icn icn-del" title="删除" data-res-action="delete" data-res-type="18">下载</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text" title="刀郎">
								  					<span title="刀郎">
								  						<a href="#" hidefocus="true">刀郎</a>
								  					</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text">
								  					<a href="#" title="谢谢你">谢谢你</a>
								  				</div>
								  			</td>
								  		</tr>
								  		<tr id="songid" class="even">
								  			<td class="left">
								  				<div class="hd">
								  					<span data-res-id="22222" data-res-type="18" data-res-action="play" class="ply">&nbsp;</span>
								  					<span class="num">3</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="f-cb">
								  					<div class="tt">
								  						<div class="ttc">
								  							<span class="txt">
								  								<a href="#">
								  									<b title="为什么我泪水成行">为什么我泪水成行</b>
								  								</a>
								  							</span>
								  						</div>
								  					</div>
								  				</div>
								  			</td>
								  			<td >
								  				<span class="u-dur candel">04:40</span>
								  				<div class="opt hshow">
								  					<a href="javascript:;" class="u-icn u-icn-81 icn-add" title="添加到播放列表" hidefocus="true" data-res-type="18" data-res-action="addto"></a>
								  					<span class="icn icn-fav" title="收藏" data-res-id="516320327" data-res-type="18" data-res-action="fav"></span>
								  					<span class="icn icn-share" data-res-action="share" title="分享" data-res-type="18" data-res-name="为什么我泪水成行" data-res-author="刀郎">分享</span>
								  					<span class="icn icn-dl" title="下载" data-res-action="download" data-res-type="18">下载</span>
								  					<span class="icn icn-del" title="删除" data-res-action="delete" data-res-type="18">下载</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text" title="刀郎">
								  					<span title="刀郎">
								  						<a href="#" hidefocus="true">刀郎</a>
								  					</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text">
								  					<a href="#" title="谢谢你">谢谢你</a>
								  				</div>
								  			</td>
								  		</tr>
								  		<tr id="songid">
								  			<td class="left">
								  				<div class="hd">
								  					<span data-res-id="22222" data-res-type="18" data-res-action="play" class="ply">&nbsp;</span>
								  					<span class="num">4</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="f-cb">
								  					<div class="tt">
								  						<div class="ttc">
								  							<span class="txt">
								  								<a href="#">
								  									<b title="为什么我泪水成行">为什么我泪水成行</b>
								  								</a>
								  							</span>
								  						</div>
								  					</div>
								  				</div>
								  			</td>
								  			<td >
								  				<span class="u-dur candel">04:40</span>
								  				<div class="opt hshow">
								  					<a href="javascript:;" class="u-icn u-icn-81 icn-add" title="添加到播放列表" hidefocus="true" data-res-type="18" data-res-action="addto"></a>
								  					<span class="icn icn-fav" title="收藏" data-res-id="516320327" data-res-type="18" data-res-action="fav"></span>
								  					<span class="icn icn-share" data-res-action="share" title="分享" data-res-type="18" data-res-name="为什么我泪水成行" data-res-author="刀郎">分享</span>
								  					<span class="icn icn-dl" title="下载" data-res-action="download" data-res-type="18">下载</span>
								  					<span class="icn icn-del" title="删除" data-res-action="delete" data-res-type="18">下载</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text" title="刀郎">
								  					<span title="刀郎">
								  						<a href="#" hidefocus="true">刀郎</a>
								  					</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text">
								  					<a href="#" title="谢谢你">谢谢你</a>
								  				</div>
								  			</td>
								  		</tr>
								  		<tr id="songid" class="even">
								  			<td class="left">
								  				<div class="hd">
								  					<span data-res-id="22222" data-res-type="18" data-res-action="play" class="ply">&nbsp;</span>
								  					<span class="num">5</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="f-cb">
								  					<div class="tt">
								  						<div class="ttc">
								  							<span class="txt">
								  								<a href="#">
								  									<b title="为什么我泪水成行">为什么我泪水成行</b>
								  								</a>
								  							</span>
								  						</div>
								  					</div>
								  				</div>
								  			</td>
								  			<td >
								  				<span class="u-dur candel">04:40</span>
								  				<div class="opt hshow">
								  					<a href="javascript:;" class="u-icn u-icn-81 icn-add" title="添加到播放列表" hidefocus="true" data-res-type="18" data-res-action="addto"></a>
								  					<span class="icn icn-fav" title="收藏" data-res-id="516320327" data-res-type="18" data-res-action="fav"></span>
								  					<span class="icn icn-share" data-res-action="share" title="分享" data-res-type="18" data-res-name="为什么我泪水成行" data-res-author="刀郎">分享</span>
								  					<span class="icn icn-dl" title="下载" data-res-action="download" data-res-type="18">下载</span>
								  					<span class="icn icn-del" title="删除" data-res-action="delete" data-res-type="18">下载</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text" title="刀郎">
								  					<span title="刀郎">
								  						<a href="#" hidefocus="true">刀郎</a>
								  					</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text">
								  					<a href="#" title="谢谢你">谢谢你</a>
								  				</div>
								  			</td>
								  		</tr>
								  		<tr id="songid">
								  			<td class="left">
								  				<div class="hd">
								  					<span data-res-id="22222" data-res-type="18" data-res-action="play" class="ply">&nbsp;</span>
								  					<span class="num">6</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="f-cb">
								  					<div class="tt">
								  						<div class="ttc">
								  							<span class="txt">
								  								<a href="#">
								  									<b title="为什么我泪水成行">为什么我泪水成行</b>
								  								</a>
								  							</span>
								  						</div>
								  					</div>
								  				</div>
								  			</td>
								  			<td >
								  				<span class="u-dur candel">04:40</span>
								  				<div class="opt hshow">
								  					<a href="javascript:;" class="u-icn u-icn-81 icn-add" title="添加到播放列表" hidefocus="true" data-res-type="18" data-res-action="addto"></a>
								  					<span class="icn icn-fav" title="收藏" data-res-id="516320327" data-res-type="18" data-res-action="fav"></span>
								  					<span class="icn icn-share" data-res-action="share" title="分享" data-res-type="18" data-res-name="为什么我泪水成行" data-res-author="刀郎">分享</span>
								  					<span class="icn icn-dl" title="下载" data-res-action="download" data-res-type="18">下载</span>
								  					<span class="icn icn-del" title="删除" data-res-action="delete" data-res-type="18">下载</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text" title="刀郎">
								  					<span title="刀郎">
								  						<a href="#" hidefocus="true">刀郎</a>
								  					</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text">
								  					<a href="#" title="谢谢你">谢谢你</a>
								  				</div>
								  			</td>
								  		</tr>
								  		<tr id="songid" class="even">
								  			<td class="left">
								  				<div class="hd">
								  					<span data-res-id="22222" data-res-type="18" data-res-action="play" class="ply">&nbsp;</span>
								  					<span class="num">7</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="f-cb">
								  					<div class="tt">
								  						<div class="ttc">
								  							<span class="txt">
								  								<a href="#">
								  									<b title="为什么我泪水成行">为什么我泪水成行</b>
								  								</a>
								  							</span>
								  						</div>
								  					</div>
								  				</div>
								  			</td>
								  			<td >
								  				<span class="u-dur candel">04:40</span>
								  				<div class="opt hshow">
								  					<a href="javascript:;" class="u-icn u-icn-81 icn-add" title="添加到播放列表" hidefocus="true" data-res-type="18" data-res-action="addto"></a>
								  					<span class="icn icn-fav" title="收藏" data-res-id="516320327" data-res-type="18" data-res-action="fav"></span>
								  					<span class="icn icn-share" data-res-action="share" title="分享" data-res-type="18" data-res-name="为什么我泪水成行" data-res-author="刀郎">分享</span>
								  					<span class="icn icn-dl" title="下载" data-res-action="download" data-res-type="18">下载</span>
								  					<span class="icn icn-del" title="删除" data-res-action="delete" data-res-type="18">下载</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text" title="刀郎">
								  					<span title="刀郎">
								  						<a href="#" hidefocus="true">刀郎</a>
								  					</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text">
								  					<a href="#" title="谢谢你">谢谢你</a>
								  				</div>
								  			</td>
								  		</tr>
								  		<tr id="songid">
								  			<td class="left">
								  				<div class="hd">
								  					<span data-res-id="22222" data-res-type="18" data-res-action="play" class="ply">&nbsp;</span>
								  					<span class="num">8</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="f-cb">
								  					<div class="tt">
								  						<div class="ttc">
								  							<span class="txt">
								  								<a href="#">
								  									<b title="为什么我泪水成行">为什么我泪水成行</b>
								  								</a>
								  							</span>
								  						</div>
								  					</div>
								  				</div>
								  			</td>
								  			<td >
								  				<span class="u-dur candel">04:40</span>
								  				<div class="opt hshow">
								  					<a href="javascript:;" class="u-icn u-icn-81 icn-add" title="添加到播放列表" hidefocus="true" data-res-type="18" data-res-action="addto"></a>
								  					<span class="icn icn-fav" title="收藏" data-res-id="516320327" data-res-type="18" data-res-action="fav"></span>
								  					<span class="icn icn-share" data-res-action="share" title="分享" data-res-type="18" data-res-name="为什么我泪水成行" data-res-author="刀郎">分享</span>
								  					<span class="icn icn-dl" title="下载" data-res-action="download" data-res-type="18">下载</span>
								  					<span class="icn icn-del" title="删除" data-res-action="delete" data-res-type="18">下载</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text" title="刀郎">
								  					<span title="刀郎">
								  						<a href="#" hidefocus="true">刀郎</a>
								  					</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text">
								  					<a href="#" title="谢谢你">谢谢你</a>
								  				</div>
								  			</td>
								  		</tr>
								  		<tr id="songid" class="even">
								  			<td class="left">
								  				<div class="hd">
								  					<span data-res-id="22222" data-res-type="18" data-res-action="play" class="ply">&nbsp;</span>
								  					<span class="num">9</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="f-cb">
								  					<div class="tt">
								  						<div class="ttc">
								  							<span class="txt">
								  								<a href="#">
								  									<b title="为什么我泪水成行">为什么我泪水成行</b>
								  								</a>
								  							</span>
								  						</div>
								  					</div>
								  				</div>
								  			</td>
								  			<td >
								  				<span class="u-dur candel">04:40</span>
								  				<div class="opt hshow">
								  					<a href="javascript:;" class="u-icn u-icn-81 icn-add" title="添加到播放列表" hidefocus="true" data-res-type="18" data-res-action="addto"></a>
								  					<span class="icn icn-fav" title="收藏" data-res-id="516320327" data-res-type="18" data-res-action="fav"></span>
								  					<span class="icn icn-share" data-res-action="share" title="分享" data-res-type="18" data-res-name="为什么我泪水成行" data-res-author="刀郎">分享</span>
								  					<span class="icn icn-dl" title="下载" data-res-action="download" data-res-type="18">下载</span>
								  					<span class="icn icn-del" title="删除" data-res-action="delete" data-res-type="18">下载</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text" title="刀郎">
								  					<span title="刀郎">
								  						<a href="#" hidefocus="true">刀郎</a>
								  					</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text">
								  					<a href="#" title="谢谢你">谢谢你</a>
								  				</div>
								  			</td>
								  		</tr>
								  		<tr id="songid">
								  			<td class="left">
								  				<div class="hd">
								  					<span data-res-id="22222" data-res-type="18" data-res-action="play" class="ply">&nbsp;</span>
								  					<span class="num">10</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="f-cb">
								  					<div class="tt">
								  						<div class="ttc">
								  							<span class="txt">
								  								<a href="#">
								  									<b title="为什么我泪水成行">为什么我泪水成行</b>
								  								</a>
								  							</span>
								  						</div>
								  					</div>
								  				</div>
								  			</td>
								  			<td >
								  				<span class="u-dur candel">04:40</span>
								  				<div class="opt hshow">
								  					<a href="javascript:;" class="u-icn u-icn-81 icn-add" title="添加到播放列表" hidefocus="true" data-res-type="18" data-res-action="addto"></a>
								  					<span class="icn icn-fav" title="收藏" data-res-id="516320327" data-res-type="18" data-res-action="fav"></span>
								  					<span class="icn icn-share" data-res-action="share" title="分享" data-res-type="18" data-res-name="为什么我泪水成行" data-res-author="刀郎">分享</span>
								  					<span class="icn icn-dl" title="下载" data-res-action="download" data-res-type="18">下载</span>
								  					<span class="icn icn-del" title="删除" data-res-action="delete" data-res-type="18">下载</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text" title="刀郎">
								  					<span title="刀郎">
								  						<a href="#" hidefocus="true">刀郎</a>
								  					</span>
								  				</div>
								  			</td>
								  			<td>
								  				<div class="text">
								  					<a href="#" title="谢谢你">谢谢你</a>
								  				</div>
								  			</td>
								  		</tr>
								  	</tbody>
								</table>
							</div>
							<div class="j-flag"></div>
						</div>
            		</div>
            		<div class="j-flag f-mgt40">
            			<div id="pinglunId">
            				<div class="u-title u-title-1">
								<h3>
									<span class="f-ff2">评论</span>
								</h3>
								<span class="sub s-fc3">
									共
									<span class="j-flag">33315</span>
									条评论
								</span>
							</div>
							<div class="m-cmmt">
								<div class="iptarea">
									<div class="head">
										<img src="../assets/img/musicTop/1.jpg"/>
									</div>
									<div class="j-flag">
										<div class="">
											<div class="m-cmmtipt f-cb f-pr">
												<div class="u-txtwrap holder-parent f-pr">
													<textarea class="u-txt area j-flag" placeholder="评论"></textarea>
												</div>
												<div class="btns f-cb f-pr">
													<i class="icn u-icn u-icn-36 j-flag"></i>
													<i class="icn u-icn u-icn-41 j-flag"></i>
													<a href="javascrit:;" class="btn u-btn u-btn-1 j-flag">评论</a>
													<span class="zs s-fc4 j-flag">140</span>
												</div>
												<div class="corr u-arr">
													<em class="arrline">◆</em>
													<span class="arrclr">◆</span>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div class="cmmts j-flag">
									<h3 class="u-hd4">精彩评论</h3>
									<div id="list1" class="itm">
										<div class="head">
											<a href="#">
												<img src="../assets/img/musicTop/1.jpg"/>
											</a>
										</div>
										<div class="cntwrap">
											<div>
												<div class="cnt f-brk">
													<a href="#" class="s-fc7">流年的刻痕Duan</a>
													：薛之谦上半年《高尚》1592％，《动物世界》1709％，《暧昧》1798％，网易云音乐飙升纪录，《别》1466％，网易云音乐今年总共三首歌飙升超过1500%，最后强推《我害怕》，歌好听，MV也很好！！！
												</div>
											</div>
											<div class="rp">
												<div class="time s-fc4">
													昨天11:14
												</div>
												<span class="dlt">
													<a href="javascript:;" class="s-fc3" data-type="delete" data-id="611103943">删除</a>
													<span class="sep">|</span>	
												</span>
												<a href="javascript:;" data-type="like">
													<i class="zan u-icn2 u-icn2-12"></i>
													(228)
												</a>
												<span class="sep">|</span>
												<a href="javascript:;" class="s-fc3" data-type="reply">回复</a>
											</div>
										</div>
									</div>
									<div id="list2" class="itm">
										<div class="head">
											<a href="#">
												<img src="../assets/img/musicTop/1.jpg"/>
											</a>
										</div>
										<div class="cntwrap">
											<div>
												<div class="cnt f-brk">
													<a href="#" class="s-fc7">流年的刻痕Duan</a>
													：薛之谦上半年《高尚》1592％，《动物世界》1709％，《暧昧》1798％，网易云音乐飙升纪录，《别》1466％，网易云音乐今年总共三首歌飙升超过1500%，最后强推《我害怕》，歌好听，MV也很好！！！
												</div>
											</div>
											<div class="rp">
												<div class="time s-fc4">
													昨天11:14
												</div>
												<a href="javascript:;" data-type="like">
													<i class="zan u-icn2 u-icn2-12"></i>
													(228)
												</a>
												<span class="sep">|</span>
												<a href="javascript:;" class="s-fc3" data-type="reply">回复</a>
											</div>
										</div>
									</div>
									<div id="list3" class="itm">
										<div class="head">
											<a href="#">
												<img src="../assets/img/musicTop/1.jpg"/>
											</a>
										</div>
										<div class="cntwrap">
											<div>
												<div class="cnt f-brk">
													<a href="#" class="s-fc7">流年的刻痕Duan</a>
													：薛之谦上半年《高尚》1592％，《动物世界》1709％，《暧昧》1798％，网易云音乐飙升纪录，《别》1466％，网易云音乐今年总共三首歌飙升超过1500%，最后强推《我害怕》，歌好听，MV也很好！！！
												</div>
											</div>
											<div class="rp">
												<div class="time s-fc4">
													昨天11:14
												</div>
												<a href="javascript:;" data-type="like">
													<i class="zan u-icn2 u-icn2-12"></i>
													(228)
												</a>
												<span class="sep">|</span>
												<a href="javascript:;" class="s-fc3" data-type="reply">回复</a>
											</div>
										</div>
									</div>
									<div id="list4" class="itm">
										<div class="head">
											<a href="#">
												<img src="../assets/img/musicTop/1.jpg"/>
											</a>
										</div>
										<div class="cntwrap">
											<div>
												<div class="cnt f-brk">
													<a href="#" class="s-fc7">流年的刻痕Duan</a>
													：薛之谦上半年《高尚》1592％，《动物世界》1709％，《暧昧》1798％，网易云音乐飙升纪录，《别》1466％，网易云音乐今年总共三首歌飙升超过1500%，最后强推《我害怕》，歌好听，MV也很好！！！
												</div>
											</div>
											<div class="rp">
												<div class="time s-fc4">
													昨天11:14
												</div>
												<a href="javascript:;" data-type="like">
													<i class="zan u-icn2 u-icn2-12"></i>
													(228)
												</a>
												<span class="sep">|</span>
												<a href="javascript:;" class="s-fc3" data-type="reply">回复</a>
											</div>
										</div>
									</div>
									<br />
									<br />
									<h3 class="u-hd4">最新评论(83514)</h3>
									<div id="list4" class="itm">
										<div class="head">
											<a href="#">
												<img src="../assets/img/musicTop/1.jpg"/>
											</a>
										</div>
										<div class="cntwrap">
											<div>
												<div class="cnt f-brk">
													<a href="#" class="s-fc7">流年的刻痕Duan</a>
													：薛之谦上半年《高尚》1592％，《动物世界》1709％，《暧昧》1798％，网易云音乐飙升纪录，《别》1466％，网易云音乐今年总共三首歌飙升超过1500%，最后强推《我害怕》，歌好听，MV也很好！！！
												</div>
											</div>
											<div class="rp">
												<div class="time s-fc4">
													昨天11:14
												</div>
												<a href="javascript:;" data-type="like">
													<i class="zan u-icn2 u-icn2-12"></i>
													(228)
												</a>
												<span class="sep">|</span>
												<a href="javascript:;" class="s-fc3" data-type="reply">回复</a>
											</div>
										</div>
									</div>
									<div id="list4" class="itm">
										<div class="head">
											<a href="#">
												<img src="../assets/img/musicTop/1.jpg"/>
											</a>
										</div>
										<div class="cntwrap">
											<div>
												<div class="cnt f-brk">
													<a href="#" class="s-fc7">流年的刻痕Duan</a>
													：薛之谦上半年《高尚》1592％，《动物世界》1709％，《暧昧》1798％，网易云音乐飙升纪录，《别》1466％，网易云音乐今年总共三首歌飙升超过1500%，最后强推《我害怕》，歌好听，MV也很好！！！
												</div>
											</div>
											<div class="rp">
												<div class="time s-fc4">
													昨天11:14
												</div>
												<a href="javascript:;" data-type="like">
													<i class="zan u-icn2 u-icn2-12"></i>
													(228)
												</a>
												<span class="sep">|</span>
												<a href="javascript:;" class="s-fc3" data-type="reply">回复</a>
											</div>
										</div>
									</div>
									<div id="list4" class="itm">
										<div class="head">
											<a href="#">
												<img src="../assets/img/musicTop/1.jpg"/>
											</a>
										</div>
										<div class="cntwrap">
											<div>
												<div class="cnt f-brk">
													<a href="#" class="s-fc7">流年的刻痕Duan</a>
													：薛之谦上半年《高尚》1592％，《动物世界》1709％，《暧昧》1798％，网易云音乐飙升纪录，《别》1466％，网易云音乐今年总共三首歌飙升超过1500%，最后强推《我害怕》，歌好听，MV也很好！！！
												</div>
											</div>
											<div class="rp">
												<div class="time s-fc4">
													昨天11:14
												</div>
												<a href="javascript:;" data-type="like">
													<i class="zan u-icn2 u-icn2-12"></i>
													(228)
												</a>
												<span class="sep">|</span>
												<a href="javascript:;" class="s-fc3" data-type="reply">回复</a>
											</div>
										</div>
									</div>
									<div id="list4" class="itm">
										<div class="head">
											<a href="#">
												<img src="../assets/img/musicTop/1.jpg"/>
											</a>
										</div>
										<div class="cntwrap">
											<div>
												<div class="cnt f-brk">
													<a href="#" class="s-fc7">流年的刻痕Duan</a>
													：薛之谦上半年《高尚》1592％，《动物世界》1709％，《暧昧》1798％，网易云音乐飙升纪录，《别》1466％，网易云音乐今年总共三首歌飙升超过1500%，最后强推《我害怕》，歌好听，MV也很好！！！
												</div>
											</div>
											<div class="rp">
												<div class="time s-fc4">
													昨天11:14
												</div>
												<a href="javascript:;" data-type="like">
													<i class="zan u-icn2 u-icn2-12"></i>
													(228)
												</a>
												<span class="sep">|</span>
												<a href="javascript:;" class="s-fc3" data-type="reply">回复</a>
											</div>
										</div>
									</div>
									<div id="list4" class="itm">
										<div class="head">
											<a href="#">
												<img src="../assets/img/musicTop/1.jpg"/>
											</a>
										</div>
										<div class="cntwrap">
											<div>
												<div class="cnt f-brk">
													<a href="#" class="s-fc7">流年的刻痕Duan</a>
													：薛之谦上半年《高尚》1592％，《动物世界》1709％，《暧昧》1798％，网易云音乐飙升纪录，《别》1466％，网易云音乐今年总共三首歌飙升超过1500%，最后强推《我害怕》，歌好听，MV也很好！！！
												</div>
											</div>
											<div class="rp">
												<div class="time s-fc4">
													昨天11:14
												</div>
												<a href="javascript:;" data-type="like">
													<i class="zan u-icn2 u-icn2-12"></i>
													(228)
												</a>
												<span class="sep">|</span>
												<a href="javascript:;" class="s-fc3" data-type="reply">回复</a>
											</div>
										</div>
									</div>
								</div>
								<div class="j-flag auto-1509759547054-parent">
									<div class="auto-1509759547054 u-page">
										<a href="javascript:;" class="zbtn zprv js-p-1509759547052 js-disabled">上一页</a>
										<a href="javascript:;" class="zpgi zpg1 js-i-1509759547052 js-selected">1</a>
										<span class="zdot on">...</span>
										<a href="javascript:;" class="zpgi zpg1 js-i-1509759547052">2</a>
										<a href="javascript:;" class="zpgi zpg1 js-i-1509759547052">3</a>
										<a href="javascript:;" class="zpgi zpg1 js-i-1509759547052">4</a>
										<a href="javascript:;" class="zpgi zpg1 js-i-1509759547052">5</a>
										<a href="javascript:;" class="zpgi zpg1 js-i-1509759547052">6</a>
										<a href="javascript:;" class="zpgi zpg1 js-i-1509759547052">7</a>
										<a href="javascript:;" class="zpgi zpg1 js-i-1509759547052">8</a>
										<span class="zdot">...</span>
										<a href="javascript:;" class="zpgi zpg1 js-i-1509759547052">1102</a>
										<a href="javascript:;" class="zbtn znxt js-p-1509759547052">下一页</a>
									</div>
								</div>
							</div>
            			</div>
            		</div>
            	</div>
            </div>
        </div>
    </div>
</template>

<style>
@import '../assets/css/myMusic.css';
</style>

